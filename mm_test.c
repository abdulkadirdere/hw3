...
int main(int argc, char **argv)
{
    ...
    mm_free(array[5]);
    printf("Test 4:the first fitting algorithm works correctly!\n");
	
    array[1] = (int*)mm_malloc(5);
    fprintf(stderr, "array[0]: %x array[1]: %x\n", array[0], array[1]);
    assert(array[0] == array[1]);
    mm_free(array[1]);
    printf("Test 5:the first fitting algorithm works correctly!\n");

    array[1] = (int*)mm_malloc(20);
    array[4] = (int*)mm_malloc(5);
    fprintf(stderr, "array[1]: %x array[4]: %x\n", array[1], array[4]);
    assert(array[1] > array[4]);
    mm_free(array[1]);
    mm_free(array[4]);
    printf("Test 6: the first fitting algorithm works correctly!\n");
    ...
    mm_free(array[0]);
    mm_free(array[2]);
    printf("Test 7: the fusion function works correctly!\n");
